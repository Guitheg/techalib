...
def benchmark_dx():
    ...
    duration = timeit.timeit(lambda: talib.DX(high, low, close, period), number=iterations)
    average_time_c = duration / iterations

    duration = timeit.timeit(lambda: tx.dx(high, low, close, period), number=iterations)

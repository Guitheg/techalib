...
use techalib::{
    ...
    indicators::plus_dm::{plus_dm, plus_dm_into, PlusDmResult, PlusDmSample},
    traits::State,
    types::Float,
};

fn generated_and_no_lookahead_plus_dm(file_name: &str, period: usize) {
    ...
    let expected = columns.get("plus_dm").unwrap();

    let high_prices_prev = &high_prices[0..last_idx];
    let low_prices_prev = &low_prices[0..last_idx];

    let output = plus_dm(high_prices_prev, low_prices_prev, period);
    assert!(
        output.is_ok(),
    ...
    assert_vec_eq_gen_data_eps(&expected[0..last_idx], &result.plus_dm, 1e-7);
    ...
    for i in 0..next_count {
        new_state
            ...
            .unwrap();
        assert!(
            approx_eq_float(new_state.prev_plus_dm, expected[last_idx + i], 1e-8),
            "Next [{}] expected {}, but got {}",
            i,
            expected[last_idx + i],
            new_state.prev_plus_dm
        );
    }
}
...
fn generated_with_no_lookahead_ok() {
    generated_and_no_lookahead_plus_dm("plus_dm.csv", 14)
}

#[test]
fn generated_with_no_lookahead_period_1_ok() {
    generated_and_no_lookahead_plus_dm("plus_dm_timeperiod-1.csv", 1)
}
...
fn finite_extreme_err_overflow_or_ok_all_finite() {
    ...
    expect_err_overflow_or_ok_with!(
        plus_dm(&high_prices, &low_prices, period),
        |result: PlusDmResult| {
            assert!(
                result.plus_dm.iter().skip(period).all(|v| v.is_finite()),
...
fn next_with_finite_neg_extreme_err_overflow_or_ok_all_finite() {
    ...
    let period = 3;
    let result = plus_dm(&high_prices, &low_prices, period).unwrap();
    let mut state = result.state;
    let plus_dm_sample = PlusDmSample {
        high: Float::MIN + 50.0,
        low: Float::MIN + 1.0,
    };
    expect_err_overflow_or_ok_with!(state.update(&plus_dm_sample), |_| {
        assert!(
            state.prev_plus_dm.is_finite(),
            "Expected all values to be finite"
        );
    });
}
...
fn unexpected_nan_err() {
    ...
    let period = 3;
    let result = plus_dm(&high_prices, &low_prices, period);
    assert!(
        result.is_err(),
...
fn different_length_input_output_err() {
    ...
    let period = 3;
    let result = plus_dm_into(&input, &low_input, period, output.as_mut_slice());
    assert!(result.is_err());
    assert!(matches!(result, Err(TechalibError::BadParam(_))));
}

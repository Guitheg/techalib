...
def benchmark_minus_di():
    ...
    duration = timeit.timeit(lambda: talib.MINUS_DI(high, low, close, period), number=iterations)
    average_time_c = duration / iterations

    duration = timeit.timeit(lambda: tx.minus_di(high, low, close, period), number=iterations)

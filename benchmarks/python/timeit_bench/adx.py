...
def benchmark_adx():
    ...
    duration = timeit.timeit(lambda: talib.ADX(high, low, close, period), number=iterations)
    average_time_c = duration / iterations

    duration = timeit.timeit(lambda: tx.adx(high, low, close, period), number=iterations)

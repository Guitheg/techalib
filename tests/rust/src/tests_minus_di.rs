...
use techalib::{
    ...
    indicators::minus_di::{minus_di, minus_di_into, MinusDiResult, MinusDiSample},
    traits::State,
    types::Float,
};

fn generated_and_no_lookahead_minus_di(file_name: &str, period: usize) {
    ...
    let expected_minus_di = columns.get("minus_di").unwrap();
    ...
    assert!(
        output.is_ok(),
        "Failed to calculate MINUS_DI: {:?}",
        output.err()
    );
    let result = output.unwrap();

    assert_vec_eq_gen_data(&expected_minus_di[0..last_idx], &result.minus_di);
    ...
    for i in 0..next_count {
        ...
        assert!(
            approx_eq_float(
                new_state.prev_minus_di,
                expected_minus_di[last_idx + i],
                1e-8
            ),
...
fn generated_with_no_lookahead_ok() {
    generated_and_no_lookahead_minus_di("minus_di.csv", 14)
}

#[test]
fn generated_with_no_lookahead_period_1_ok() {
    generated_and_no_lookahead_minus_di("minus_di_timeperiod-1.csv", 1)
}
...
fn finite_extreme_err_overflow_or_ok_all_finite() {
    ...
    expect_err_overflow_or_ok_with!(
        minus_di(&data, &data, &data, period),
        |result: MinusDiResult| {
            assert!(
                result.minus_di.iter().skip(period).all(|v| v.is_finite()),
...
fn next_with_finite_neg_extreme_err_overflow_or_ok_all_finite() {
    ...
    let period = 3;
    let result = minus_di(&data, &data, &data, period).unwrap();
...
fn unexpected_nan_err() {
    ...
    let period = 3;
    let result = minus_di(&data, &data, &data, period);
...
fn different_length_input_output_err() {
    ...
    let period = 3;
    let result = minus_di_into(&data, &data, &data, period, output.as_mut_slice());
    assert!(result.is_err());
